  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="no-print flex flex-col sm:flex-row gap-4 items-start sm:items-center">
        <div>
          <h1 className="font-display font-bold text-2xl text-gray-900">Print Design Sheet</h1>
      ...
      {order && (
        ...
        >
          ...
          <div className="flex items-start justify-between mb-6 pb-5 border-b border-surface-border print:border-gray-300">
            <div>
              <h2 className="font-display font-bold text-2xl text-gray-900 print:text-black">Timelines Costume Designers</h2>
              <p className="text-gray-600 text-sm print:text-gray-600">Design Work Sheet</p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div className="card-glass print:bg-gray-50 print:border-gray-200 p-4 rounded-xl">
              ...
              <p className="font-mono font-bold text-gray-900 text-lg print:text-black">{order.orderId}</p>
            </div>
            <div className="card-glass print:bg-gray-50 print:border-gray-200 p-4 rounded-xl">
              <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Garment Type</p>
              <p className="font-bold text-gray-900 print:text-black">{order.garmentType}</p>
            </div>
            <div className="card-glass print:bg-gray-50 print:border-gray-200 p-4 rounded-xl">
            ...
            <div className="card-glass print:bg-gray-50 print:border-gray-200 p-4 rounded-xl">
              ...
              <p className="text-gray-900 print:text-black">{order.orderDate ? format(new Date(order.orderDate), 'dd MMMM yyyy') : '-'}</p>
            </div>
            <div className="card-glass print:bg-gray-50 print:border-gray-200 p-4 rounded-xl">
              <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Assigned Tailor</p>
              <p className="text-gray-900 print:text-black">{order.tailor?.name || order.assignedTailorId?.name || '-'}</p>
            </div>
          </div>
          ...
          {Object.keys(measurements).length > 0 && (
            <div className="mb-6">
              <h3 className="font-semibold text-gray-900 print:text-black mb-3 text-base">Measurements</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {Object.entries(measurements).map(([key, val]) => (
                  <div key={key} className="flex items-center justify-between bg-surface-elevated print:bg-gray-50 rounded-lg px-3 py-2 text-sm">
                    <span className="text-gray-600 print:text-gray-600 capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                    <span className="font-semibold text-gray-900 print:text-black">{val}"</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {order.designSketchUrl && (
            <div className="mb-6">
              <h3 className="font-semibold text-gray-900 print:text-black mb-3 text-base">Design Sketch</h3>
          ...
          {order.fabricNotes && (
            <div className="mb-4">
              <h3 className="font-semibold text-gray-900 print:text-black mb-2 text-base">Fabric Notes</h3>
              <div className="bg-surface-elevated print:bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-800 print:text-gray-700">
                {order.fabricNotes}
              </div>
            </div>
          )}
          ...
          {order.specialInstructions && (
            <div className="mb-4">
              <h3 className="font-semibold text-gray-900 print:text-black mb-2 text-base">Special Instructions</h3>
